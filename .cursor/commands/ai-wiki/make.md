...
5. ページ間リンクの生成
6. ファイル出力と完了報告
7. サイドバーファイル（`_sidebar.md`）の生成（Wikiページ生成の完了後）
...
## サイドバー生成機能
Wikiページ生成の完了後、`wiki_structure.json`のセクション構造を基にサイドバーファイル（`_sidebar.md`）を自動生成します。サイドバーは標準的なMarkdown形式で生成され、GitBookやDocsifyなどの一般的なWikiシステムで利用可能です。サイドバー生成の失敗は報告されますが、Wikiページ生成の結果には影響を与えません。
</background_information>
...
## Step 5: 完了報告（Wikiページ生成）

1. Wikiページ生成の完了時に、生成されたページ数と失敗したページ数（該当する場合）を報告します。
   - 「Wikiページの生成が完了しました。生成されたページ数: ${successCount}、失敗したページ数: ${failureCount}」というメッセージを報告します
   - 生成されたファイルのパス（`${outputDir}/`）をユーザーに報告します

## Step 6: サイドバーファイルの生成

1. Wikiページ生成の完了後、サイドバーファイル（`_sidebar.md`）を生成します。
   - サイドバー生成の成功/失敗を追跡するために、`${sidebarSuccess}` 変数を初期化します（初期値: `false`）
   - サイドバー生成のエラーを追跡するために、`${sidebarError}` 変数を初期化します（初期値: `null`）

2. セクション構造の判定を行います。
   - `${sections}` 配列が存在し、かつ空でない場合: セクション構造に基づくナビゲーションを生成します（次のステップ3に進みます）
   - `${sections}` 配列が存在しない、または空の場合（簡潔ビュー）: 全ページをフラットなリストとして生成します（ステップ4に進みます）

3. セクション構造に基づくナビゲーションを生成します。
   - サイドバーのMarkdownコンテンツを格納するために、`${sidebarContent}` 変数を初期化します（初期値: 空文字列）
   - `${sections}` 配列をループ処理して、各セクションごとに処理します
   - 各セクションオブジェクトを `${section}` 変数として設定します
   - 各セクションの`title`をセクション見出しとして使用し、`${sidebarContent}` に追加します（形式: `## ${section.title}\n\n`）
   - 各セクションの`pages`配列をループ処理して、各ページIDに対して処理します
     - ページIDを `${pageId}` 変数として設定します
     - `${pages}` 配列から該当するページオブジェクトを検索します（`id`が `${pageId}` と一致するページ）
     - ページオブジェクトが見つかった場合:
       - ページの`title`を取得し、`${pageTitle}` 変数として設定します
       - ページタイトルが存在しない場合は、ページIDを `${pageTitle}` として使用します（フォールバック）
       - ページリンクを生成します（形式: `- [${pageTitle}](${pageId})\n`）
       - 生成されたページリンクを `${sidebarContent}` に追加します
     - ページオブジェクトが見つからない場合: そのページIDをスキップします（エラーは報告しません）
   - セクション間の可読性向上のため、セクションの最後に空行を追加します（`${sidebarContent}` に `\n` を追加）
   - すべてのセクションの処理が完了したら、次のステップ5に進みます

4. 簡潔ビュー（sections配列が存在しない場合）の処理を実行します。
   - サイドバーのMarkdownコンテンツを格納するために、`${sidebarContent}` 変数を初期化します（初期値: 空文字列）
   - `${pages}` 配列をループ処理して、各ページオブジェクトに対して処理します
   - 各ページオブジェクトを `${page}` 変数として設定します
   - ページの`title`を取得し、`${pageTitle}` 変数として設定します
   - ページタイトルが存在しない場合は、ページIDを `${pageTitle}` として使用します（フォールバック）
   - ページリンクを生成します（形式: `- [${pageTitle}](${page.id})\n`）
   - 生成されたページリンクを `${sidebarContent}` に追加します
   - すべてのページの処理が完了したら、次のステップ5に進みます

5. サイドバーファイルを書き込みます。
   - `write` ツールを使用して、生成されたMarkdownコンテンツ（`${sidebarContent}`）を `${outputDir}/_sidebar.md` に書き込みます
   - ファイル書き込みに失敗した場合:
     - エラーメッセージ「サイドバーファイル `${outputDir}/_sidebar.md` の書き込みに失敗しました。」を報告します
     - `${sidebarError}` 変数にエラーメッセージを設定します
     - `${sidebarSuccess}` 変数は `false` のままにします
     - Wikiページ生成の結果には影響を与えず、処理を継続します
   - ファイル書き込みが成功した場合:
     - `${sidebarSuccess}` 変数を `true` に設定します
     - `${sidebarError}` 変数は `null` のままにします

## Step 7: 完了報告（サイドバー生成）

1. サイドバー生成の完了時に、成功/失敗を報告します。
   - サイドバー生成が成功した場合（`${sidebarSuccess}` が `true` の場合）:
     - 「サイドバーファイルの生成が完了しました。生成されたファイル: `${outputDir}/_sidebar.md`」というメッセージを報告します
   - サイドバー生成が失敗した場合（`${sidebarError}` が `null` でない場合）:
     - `${sidebarError}` に設定されたエラーメッセージを報告します
     - 「Wikiページの生成は正常に完了しています。」というメッセージを追加で報告します
...
2. **サイドバーファイル**: ナビゲーション用のサイドバーファイル（`_sidebar.md`）
   - 出力先ディレクトリ: `${outputDir}/`（Wikiページと同じディレクトリ）
   - ファイル名: `_sidebar.md`
   - 形式: Markdown形式、UTF-8エンコーディング
   - 内容: セクション構造に基づく階層的なナビゲーション（セクション見出しとページリンク）
   - ページリンク形式: `- [Page Title](page-id)`（`.md`拡張子は含めない）
   - セクション見出し形式: `## Section Title`

3. **完了報告**: 処理が正常に完了した場合、または一部のページの生成に失敗した場合、ユーザーに完了メッセージを報告します
...
6. **サイドバー生成エラー**:
   - サイドバーファイルの書き込みに失敗した場合、エラーメッセージを報告し、サイドバー生成をスキップします（Wikiページ生成は継続します）
   - セクション構造の解析エラーが発生した場合、エラーメッセージを報告し、サイドバー生成をスキップします（Wikiページ生成は継続します）
   - サイドバー生成の失敗は報告されますが、Wikiページ生成の結果には影響を与えません
